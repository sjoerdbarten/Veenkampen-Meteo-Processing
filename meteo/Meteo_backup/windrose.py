...
class WindroseAxes(PolarAxes):
    ...
    def legend(self, loc="lower left", decimal_places=1, **kwargs):
        ...
        def get_labels(decimal_places=1):
            ...
            labels = [fmt % (labels[i], labels[i + 1]) for i in range(len(labels) - 1)][:-1]
            #print('labels: ', labels)
            return labels
...
def histogram(direction, var, bins, nsector, normed=False, blowto=False):
    ...
    dir_bins = np.arange(-angle / 2, 360. + angle, angle, dtype=float)     #2023-03-03 KvdD changed from np.float in float numpy version change
    ...
    table = histogram2d(x=var, y=direction, bins=[var_bins, dir_bins])[0] # 2023-03-03 KvdD TypeError: histogram2d() got an unexpected keyword argument 'normed' <, normed=False> removed
